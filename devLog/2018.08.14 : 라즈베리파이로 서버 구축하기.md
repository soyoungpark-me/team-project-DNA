...
- **개발하면서 공부한 내용**
  ...
  | 웹/HTTP | 네트워크 | [네트워크\_02\_0.0.0.0과 127.0.0.1.의 차이는?]() |
...
그리고 마주친 문제는, 라즈베리파이의 유동 ip 문제였습니다. 집에서 쓸 때는 와이파이에 연결해서 써야 했는데, 이때까지만 해도 유동ip와 고정ip에 대한 명확한 개념이 없어 큰 문제가 될 거라는 생각을 못했지만... 막상 환경을 구축하다 보니 이런 문제도 발생할 수 있다는 것을 알았습니다. 이를 해결하기 위해, iptime 공유기에서 제공하는 DDNS를 이용했습니다.
